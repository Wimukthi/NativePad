...
constexpr wchar_t kProgId[] = L"NativePad.Text";
constexpr wchar_t kLegacyTxtProgId[] = L"NativePad.txt";
...
bool RegisterCapabilities(const std::wstring& exe, std::wstring& error) {
    ...
    // The canonical ProgID is shared by all supported text extensions. Keep
    // the old .txt ProgID registered as an alias so users who selected NativePad
    // before this catalog existed still show as associated.
    bool ok =
        SetRegString(HKEY_CURRENT_USER, L"Software\\Classes\\NativePad.Text", nullptr, L"NativePad Text Document") &&
        SetRegString(HKEY_CURRENT_USER, L"Software\\Classes\\NativePad.Text\\DefaultIcon", nullptr, iconValue) &&
        SetRegString(HKEY_CURRENT_USER, L"Software\\Classes\\NativePad.Text\\shell\\open\\command", nullptr, openCommand) &&
        ...
        SetRegString(HKEY_CURRENT_USER, kCapabilitiesKey, L"ApplicationIcon", iconValue);

    for (const TextFileType& type : SupportedTextFileTypes()) {
        if (!ok) {
            break;
        }

        const std::wstring extension(type.extension);
        const std::wstring openWithKey = L"Software\\Classes\\" + extension + L"\\OpenWithProgids";
        ok =
            // Register as an Open With candidate, never as the user's default.
            SetRegString(
                HKEY_CURRENT_USER,
                openWithKey.c_str(),
                kProgId,
            ...
            SetRegString(
                HKEY_CURRENT_USER,
                L"Software\\Classes\\Applications\\NativePad.exe\\SupportedTypes",
                type.extension.data(),
                L"") &&
            SetRegString(
                HKEY_CURRENT_USER,
                L"Software\\NativePad\\Capabilities\\FileAssociations",
                type.extension.data(),
                kProgId);

        if (ok && _wcsicmp(type.extension.data(), L".txt") == 0) {
            // Keep the legacy ProgID visible in .txt Open With lists.
            ok = SetRegString(
                HKEY_CURRENT_USER,
                openWithKey.c_str(),
                kLegacyTxtProgId,
    ...
    ok = ok && SetRegString(HKEY_CURRENT_USER, L"Software\\RegisteredApplications", kRegisteredAppName, kCapabilitiesKey);
...
bool IsDefaultEditorForExtension(std::wstring_view extension) {
    if (TextFileTypeForExtension(extension) == nullptr) {
        return false;
    }
    ...
    const std::wstring extensionCopy(extension);
    if (SUCCEEDED(registration->QueryCurrentDefault(extensionCopy.c_str(), AT_FILEEXTENSION, AL_EFFECTIVE, &currentProgId)) &&
        currentProgId != nullptr) {
        isDefault =
            CompareStringOrdinal(currentProgId, -1, kProgId, -1, TRUE) == CSTR_EQUAL ||
            (CompareStringOrdinal(extensionCopy.c_str(), -1, L".txt", -1, TRUE) == CSTR_EQUAL &&
             CompareStringOrdinal(currentProgId, -1, kLegacyTxtProgId, -1, TRUE) == CSTR_EQUAL);
...
bool IsDefaultEditor() {
    return IsDefaultEditorForExtension(L".txt");
}
